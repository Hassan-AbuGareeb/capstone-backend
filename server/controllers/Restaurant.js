...
const searchRestaurantItems = async (req, res) => {
    try {
     ...
     return res.status(200).json({ message: 'Search Results', items: searchResults });
 } catch (err) {
...
async function addItem(req, res){
    ...
    try{
        ...
        const existingItem = await Item.findOne({ name: item.name, restaurantId: restaurantId });
        if (existingItem) {
...
async function updateMenuItem(req, res){
    ...
    try{
        ...
        const foundItem = await Item.findOne({ restaurantId: restaurantId, name: item.name });
        if (!foundItem) {
            return res.status(404).json({ message: 'Item not found' });
        }
        console.log(item.price)
        console.log(foundItem.price.toString())
        if (item.name === foundItem.name || item.description === foundItem.description, item.price.toString() === foundItem.price.toString()) {
        ...
        const updatedItem = await Item.findByIdAndUpdate(foundItem._id, item, {new: true});
        return res.status(201).json({message: 'Item updated successfully', updatedItem});
    }
    catch(err){
        return res.status(422).json(err.message);
    }
